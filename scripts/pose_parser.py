...
class MarvelmindPoseParser():
    ...
    def __init__(self, pos_sub_topic, wheel_odom_sub_topic, pub_topic_name, frame_id, ch_frame, pose_cov, min_speed_cov, max_speed_cov):
        ...
        if self.min_cov < self.max_cov:
            rospy.logerr("Covariance for min linear speed smaller than covarianve for max linear speed.\n Covariance for min: %f\n Covariance for max: %f", self.min_cov, self.max_cov)
            return
        
        # Publisher of parsed message
        self.odom_publisher = rospy.Publisher(pub_topic_name, nav.Odometry, queue_size=1)
        ...
        self.wheel_sub = rospy.Subscriber(wheel_odom_sub_topic, geo.TwistWithCovarianceStamped, self.wheel_odom_callback)
    ...
    def make_pose(self):
        ...
        if abs(speed) < 0.2 or not self.has_history:
            ...
            self.pose_cov[-1] = self.max_cov - (abs(speed) * (self.max_cov - self.min_cov) / 0.4)
...
if __name__ == '__main__':
    ...
    wheel_odom_topic = rospy.get_param("~wheel_covariance_topic", "wheel_odom_with_covariance")
    ...
    pose_covariance_diagonal = rospy.get_param("~pose_covariance_diagonal", [0.001, 0.001, 0, 0, 0, 0])
    ...
    pose_parser = MarvelmindPoseParser(pos_topic, wheel_odom_topic, pose_pub_topic, frame, ch_frame, pose_covariance_diagonal, min_covariance, max_covariance)
    rospy.spin()    
